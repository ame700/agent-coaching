...
export class OpenMessageHandler implements MessageHandler {
    handleMessage(message: ClientMessage, session: Session) {
        ...
        console.log("open transaction parsed message :" + JSON.stringify(parsedMessage));
        session.setConversationId(parsedMessage.parameters.conversationId);



        if (parsedMessage.parameters.participant.ani != 'tel:+971543306217') {
            const message = 'Not supported call .';
            console.log(message);
            session.sendDisconnect('error', message, {});
            return;
        }

        session.registerUserId(parsedMessage.parameters.participant.id);
